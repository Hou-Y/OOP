	...
	public String topThreeStudents() {
		...
		for(i=0; i<numTopStu; i++){
			for(j=0; j<numTopStu; j++){
				if(allAvgs[i] == listTop[j].getScore()){
			...
			//ma ora la complessità rimane quadratica ffkers
				for(j=0; j<numTopStu; j++){
					if(i ==listTop[j].getPosition())
						threeTops = threeTops.append(listTop[j].getName()).append(" ").append(listTop[j].getSurName()).append(" : ").append(allAvgs[i]).append("\n");
